...
execute as @a if score @s damage_taken matches 1.. at @s at @e[tag=Entity,distance=..3] run summon armor_stand ~ ~ ~ {Tags:["ATK"],Invisible:1,Marker:1,NoGravity:1}
execute at @a[scores={damage_taken=1..}] as @e[tag=Entity,distance=..3] at @s run tp @e[tag=ATK,distance=..1,sort=nearest,limit=1] ^ ^ ^-0.01 facing entity @s
execute as @e[tag=Entity] at @s store result score @e[tag=ATK,distance=..1,sort=nearest,limit=1] ATK run data get entity @s HandItems[].tag.ATK
